...
const OurGlobalAssociates = () => {
  ...
  const logos = [
    { name: 'GROVE', src: '/logos/grove.png' },
    { name: 'HYSTER', src: '/logos/hyster.png' },
    { name: 'MANITOWOC', src: '/logos/manitowoc.png' },
    { name: 'SNORKEL', src: '/logos/snorkel.png' },
  ];
  ...
  return (
    <section className="relative bg-cover bg-center py-16 overflow-hidden">
      ...
      <div 
        ...
        style={{
          backgroundImage: "url('/src/assets/global.jpg')",
          ...
          >
            {logos.map((logo, index) => (
              <motion.div
                ...
                className="flex items-center justify-center p-3 bg-white/10 backdrop-blur-sm rounded-lg border border-white/20 hover:bg-white/20 transition-all duration-300"
              >
                <img
                  src={logo.src}
                  alt={`${logo.name} logo`}
                  className="h-10 mx-auto object-contain filter brightness-0 invert opacity-80 hover:opacity-100 transition-opacity duration-300"
                  onError={(e) => {
